...
var handleResults = function(results) {
  ...
  results.forEach(function(result) {
    ...
    var newToken = result[gcm.Constants.TOKEN_CANONICAL_REG_ID];
    if (!!newToken) {
      logger.debug('Token %s will be replaced by %s', result.token, newToken);
      idsToUpdate.push({from: result.token, to: newToken});
    } else if (result.error === 'InvalidRegistration' || result.error === 'NotRegistered') {
      logger.debug('Token %s will be deleted', result.token);
